...
AGOG.imputes.graph <- mice(AGOG.imputes.graph, m=5, maxit=5, seed=123, 
...
#' @param min_imp Cut-off of minimum number of imputations
#' @param m The number of datasets to show on the graph. Recommended value < 10 for visibility 
#' @param maxit Iterations for each dataset. Recommended value > 20 
#' @return The imputed data
#' @examples
#' @export
...
check_distribution <- function(data, cols, m = 10, ...) {
  ...
  #Same functions as above except for maxit
